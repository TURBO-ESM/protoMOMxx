...
// which is constructed from an injected RuntimeParams object.
//
// The vertical grid carries no AMReX fields, so no infra bring-up is needed and
// the tests link the stock gtest_main.
...
// The gprime coordinate from the driving testcase's parameter file (reused here
// rather than duplicated): the surface interface takes GFS, the internal
...
// GFS defaults to G_EARTH and LIGHTEST_DENSITY defaults to RHO_0; with NK = 1
// there are no internal interfaces to fill.
...
// for the value checks) are fatal.
