...
TEST(MOMGetInputTest, ReadNamelist) {
  ...
  ASSERT_NO_THROW(
    dirs = get_directories(test_file_path, -1);
...
TEST(MOMGetInputTest, EnsembleNumber) {
  ...
  ASSERT_NO_THROW(
    dirs = get_directories(test_file_path, 5);
    param_files = get_parameter_filenames(test_file_path, 5);
  );
...
TEST(MOMGetInputTest, MissingNamelistFile) {
  ...
  EXPECT_THROW(
    get_directories(test_file_path, -1),
    std::runtime_error
  );
}
