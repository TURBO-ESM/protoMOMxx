...
Domain::Domain(const std::size_t ni_global, const std::size_t nj_global,
    ...
    : ni_global(ni_global), nj_global(nj_global), ni_halo(ni_halo),
      nj_halo(nj_halo), reentrant_i(reentrant_i), reentrant_j(reentrant_j),
      tripolar_N(tripolar_N) {

  if (reentrant_j && tripolar_N) {
    throw std::invalid_argument("Invalid domain configuration: reentrant_j and "
                                "tripolar_N cannot both be true.");
  }
}
