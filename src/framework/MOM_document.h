...
public:
  ...
  /// @brief Finalize documentation: close the current module (if any) and close all files.
  ///
  ...
  template <typename T>
  void doc_param(std::string_view varname, std::string_view desc, std::string_view units, const T &val,
                 std::optional<T> default_val = std::nullopt, const DocParamOptions &opts = {});

  /// @brief Document a vector parameter (vector of bool, int, double, or std::string).
  /// @param varname The name of the parameter variable.
  /// @param desc A description of the parameter (written to doc file).
  /// @param units The units of the parameter (written to doc file).
  ...
  /// @param opts Additional options controlling documentation behavior (e.g. which files to write to).
  template <typename T>
  void doc_param(std::string_view varname, std::string_view desc, std::string_view units,
                 const std::vector<T> &vals, std::optional<std::vector<T>> default_val = std::nullopt,
  ...
  void doc_module(std::string_view modname, std::string_view desc, bool layout_mod = false,
...
private:
  ...
  std::string define_string(std::string_view varname, std::string_view valstring, std::string_view units) const;

  /// @brief Format "VARNAME = False   !   [units]" (for false booleans)
  std::string undef_string(std::string_view varname, std::string_view units) const;

  /// @brief Write a message line + wrapped description to the appropriate files.
  void write_message_and_desc(std::string_view mesg, std::string_view desc, bool value_was_default = false,
  ...
  bool mesg_has_been_documented(std::string_view varname, std::string_view mesg);
...
private:
  ...
  bool current_module_all_default_ = false; ///< Whether all params in the current module equal defaults
