...
/// @brief Parse a quoted string, ensuring it is well-formed and stripping the
/// quotes.
...
std::string parse_quoted_string(std::string_view s, std::size_t line_no,
...
/// @return The parsed (optional) boolean value if parsing was successful,
...
/// @param s The input string view to parse.
/// @return The parsed (optional) integer value if parsing was successful,
/// std::nullopt otherwise.
...
/// @brief Try to parse a string as a double using std::stod, ensuring the
...
std::optional<double> try_parse_double(std::string_view s) {
  ...
  // since std::from_chars for double doesn't seem to be fully supported in all
  // compilers yet.
  try {
    std::size_t pos = 0;
...
/// @brief Parse a scalar value from a string, trying bool, int, double, and
/// quoted string (in that order).
/// @param s The input string view to parse.
/// @param line_no The line number for error reporting.
/// @param path The file path for error reporting.
/// @return The parsed value as a ParamValue variant.
ParamValue parse_scalar(std::string_view s, std::size_t line_no,
                        const std::string &path) {
  s = trim(s);
  if (s.empty()) {
    throw std::runtime_error(path + ":" + std::to_string(line_no) +
  ...
  if (s.find_first_of(" \t\r\n") != std::string_view::npos) {
    throw std::runtime_error(path + ":" + std::to_string(line_no) +
                             ": unquoted string contains whitespace: '" +
                             std::string(s) + "'");
  }
  return std::string(s);
}

/// @brief Convert a vector of ParamValue scalars to a vector of a specific type
/// T
/// @tparam T The target type to convert to (must be one of the scalar types
/// supported by ParamValue).
...
ParamValue get_value(std::string_view raw, std::size_t line_no,
                     const std::string &path) {
  raw = trim(raw);
  if (raw.empty()) {
    throw std::runtime_error(path + ":" + std::to_string(line_no) +
  ...
  switch (idx) {
  case 1:
    return convert_vector<bool>(scalars);
  case 2:
    return convert_vector<int>(scalars);
  case 3:
    return convert_vector<double>(scalars);
  case 4:
    return convert_vector<std::string>(scalars);
  default:
    throw std::runtime_error(path + ":" + std::to_string(line_no) +
                             ": internal error parsing list");
  }
}
