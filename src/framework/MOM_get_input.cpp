...
/// @copydoc MOM_get_input::get_directories(const std::filesystem::path&, int)
Directories get_directories(const fs::path& namelist_file, int ensemble_num) {
  NamelistParams nml(namelist_file);
  Directories dirs;

  auto get_str = [&](const char* key) -> std::string {
      return nml.has_param(key, "MOM_INPUT_NML")
          ? nml.get_as<std::string>(key, "MOM_INPUT_NML")
          : std::string{};
  };
...
/// @copydoc MOM_get_input::get_parameter_filenames(const std::filesystem::path&, int)
std::vector<std::string> get_parameter_filenames(const fs::path& namelist_file, int ensemble_num) {
  ...
  if (parameter_filenames.empty()) {
    throw std::runtime_error(
        ...
        "MOM_input_nml in: " + namelist_file.string());
  }
