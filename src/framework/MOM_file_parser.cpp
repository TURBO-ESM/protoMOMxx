...
ParsedTarget parse_target(std::string_view lhs, std::string_view curr_block,
  ...
  if (pct != std::string_view::npos) {
    ...
    if (key.find('%') != std::string_view::npos) {
      throw std::runtime_error(path + ":" + std::to_string(line_no) +
                               ": only one '%' allowed in block%key: '" +
                               std::string(lhs) + "'");
    }

    if (!is_valid_identifier(blk) || !is_valid_identifier(key)) {
      throw std::runtime_error(path + ":" + std::to_string(line_no) +
                               ": invalid block%key syntax: '" +
                               std::string(lhs) + "'");
    }
  ...
  if (!is_valid_identifier(lhs)) {
    throw std::runtime_error(path + ":" + std::to_string(line_no) +
                             ": invalid key name: '" + std::string(lhs) + "'");
  }
...
/// @param in_block Reference to a boolean indicating whether parsing is
/// currently
...
std::string strip_block_comments(std::string_view line, bool &in_block) {
  ...
  for (std::size_t i = 0; i < line.size();) {
    ...
    if (!in_sq && !in_dq && c == '/' && (i + 1) < line.size() &&
        line[i + 1] == '*') {
      in_block = true;
      i += 2;
      continue;
...
/// @brief Parse a runtime parameter file and add its contents to the provided
/// table.
...
void add_data_from_file(const std::string &path, ParamTable &table) {
  ...
  if (!fs::is_regular_file(path)) {
    throw std::runtime_error("Runtime parameter path is not a regular file: " +
                             path);
  }

  std::ifstream infile(path);
  if (!infile) {
    throw std::runtime_error("Unable to open runtime parameter file at " +
                             path);
  }
  ...
  auto assign_param = [&](const std::string &block, const std::string &key,
  ...
  while (std::getline(infile, raw_line)) {
    ...
    // Strip block and line comments in a quote-aware way (handles /*...*/
    // across lines + ! inline).
    ...
    if (sv.back() == '%' && sv.front() != '%') {
      ...
      if (!is_valid_identifier(name)) {
        throw std::runtime_error(path + ":" + std::to_string(line_no) +
                                 ": invalid block name: '" + std::string(name) +
                                 "'");
      }
      if (!curr_block.empty()) {
        throw std::runtime_error(path + ":" + std::to_string(line_no) +
                                 ": cannot open block '" + std::string(name) +
    ...
    if (sv.front() == '%' && sv.size() >= 2) {
      ...
      if (curr_block != name) {
        throw std::runtime_error(path + ":" + std::to_string(line_no) +
                                 ": block mismatch: closing '" +
                                 std::string(name) + "' but open block is '" +
                                 curr_block + "'");
      }
      curr_block.clear();
      continue;
    ...
    if (sv.front() == '#') {
      ...
      if (rest.empty()) {
        throw std::runtime_error(path + ":" + std::to_string(line_no) +
                                 ": malformed directive: '" + std::string(sv) +
                                 "'");
      }
      auto cmd_end = rest.find_first_of(" \t");
      std::string cmd = lowercase(trim(
          cmd_end == std::string_view::npos ? rest : rest.substr(0, cmd_end)));
      auto args =
          trim(cmd_end == std::string_view::npos ? std::string_view{}
                                                 : rest.substr(cmd_end + 1));
      if (args.empty()) {
        throw std::runtime_error(path + ":" + std::to_string(line_no) +
                                 ": directive '#" + std::string(cmd) +
                                 "' requires arguments");
      }

      auto lhs_end = args.find_first_of(" \t\r");
      auto lhs = trim(
          lhs_end == std::string_view::npos ? args : args.substr(0, lhs_end));
      if (lhs.empty()) {
        throw std::runtime_error(path + ":" + std::to_string(line_no) +
                                 ": directive '#" + std::string(cmd) +
                                 "' requires a key");
      ...
      auto rhs =
          trim(lhs_end == std::string_view::npos ? std::string_view{}
                                                 : args.substr(lhs_end + 1));
      ...
      if (cmd == "define") {
        if (rhs.empty()) {
          ...
          if (rhs.front() == '=') {
            throw std::runtime_error(
                path + ":" + std::to_string(line_no) +
                ": #define does not use '='; write '#define " + target.key +
                " <value>' or '" + target.key + " = <value>'");
          }
          // #define key value means parse value as usual
          assign_param(target.block, target.key, get_value(rhs, line_no, path),
                       false);
        }
        continue;
      }
      ...
      if (cmd == "undef") {
        if (!rhs.empty()) {
          throw std::runtime_error(
              path + ":" + std::to_string(line_no) +
              ": #undef takes only a key name, but got trailing content: '" +
              std::string(rhs) + "'");
        }
        bool is_override = false;
        // if a value was already provided, it must be "true":
        if (table.has_param(target.key, target.block)) {
          const auto &existing_val =
              table.get_variant(target.key, target.block);
          if (!std::holds_alternative<bool>(existing_val) ||
              !std::get<bool>(existing_val)) {
            throw std::runtime_error(path + ":" + std::to_string(line_no) +
                                     ": #undef can only be applied to keys "
                                     "that are currently defined as true; use "
      ...
      throw std::runtime_error(path + ":" + std::to_string(line_no) +
                               ": unknown directive '#" + std::string(cmd) +
                               "'");
    }
  ...
  if (in_block_comment) {
    throw std::runtime_error(path +
                             ":EOF: unterminated block comment (missing '*/')");
  }
  if (!curr_block.empty()) {
    throw std::runtime_error(path + ":EOF: unterminated block '" + curr_block +
                             "' (missing '%" + curr_block + "')");
  }
}
...
RuntimeParams::RuntimeParams(const std::string &path,
...
void RuntimeParams::open_block(const std::string &blockName,
                               const std::string &desc) {
  if (blockName.empty() || !string_utils::is_valid_identifier(blockName)) {
    throw std::invalid_argument("open_block: invalid block name \"" +
                                blockName + "\"");
  }
  if (!current_block_.empty()) {
    throw std::logic_error("open_block(\"" + blockName +
                           "\") called but block \"" + current_block_ +
                           "\" is already open");
  }
  current_block_ = blockName;
  if (doc_)
    doc_->open_block(blockName, desc);
}
...
void RuntimeParams::doc_module(const std::string &modname,
                               const std::string &desc, bool layout_mod,
  ...
  if (doc_) {
    ...
    logger::warning("Attempted to document module '", modname,
                    "' but no documentation writer is attached. Provide a "
                    "non-empty doc_file_base when constructing RuntimeParams "
...
void RuntimeParams::close_module() {
  if (doc_) {
    ...
    logger::warning("Attempted to close module documentation but no "
                    "documentation writer is attached. "
                    "Provide a non-empty doc_file_base when constructing "
                    "RuntimeParams to enable documentation.");
  }
}

template <typename T>
void RuntimeParams::get(const std::string &key, T &value,
                        const ParamGetOptions &options) const {
  bool value_was_set = false;
  auto full_key = [&]() {
    return current_block_.empty() ? key : current_block_ + "%" + key;
  };
  ...
  if (!options.do_not_read) {
    try {
      ...
      if (std::holds_alternative<T>(val)) {
        ...
        throw std::runtime_error("Parameter " + full_key() +
                                 " is not of the requested type");
      }
    } catch (const std::out_of_range &) {
      if (options.fail_if_missing) {
        throw std::out_of_range("Key not found" +
                                (current_block_.empty()
                                     ? std::string{}
                                     : " in block " + current_block_) +
                                ": " + key);
      }
    }
  }
  ...
  if (!value_was_set && options.default_value.has_value()) {
    if (!std::holds_alternative<T>(options.default_value.value())) {
      throw std::runtime_error("Default value for " + full_key() +
                               " is not of the requested type");
    }
  ...
  if (!value_was_set && !options.do_not_read) {
    logger::warning("Parameter ", full_key(),
                    " was not found and no default value was provided. "
  ...
  if (value_was_set && doc_ && !options.do_not_log && !options.desc.empty()) {
    ...
    if (options.default_value.has_value() &&
        std::holds_alternative<T>(options.default_value.value())) {
      typed_default = std::get<T>(options.default_value.value());
    }

    doc_->doc_param(key, options.desc, options.units, value, typed_default,
                    doc_opts);
  }
}

// Explicit template instantiations for get
template void RuntimeParams::get<bool>(const std::string &, bool &,
                                       const ParamGetOptions &) const;
template void RuntimeParams::get<int>(const std::string &, int &,
                                      const ParamGetOptions &) const;
template void RuntimeParams::get<double>(const std::string &, double &,
                                         const ParamGetOptions &) const;
template void RuntimeParams::get<std::string>(const std::string &,
                                              std::string &,
                                              const ParamGetOptions &) const;
template void
RuntimeParams::get<std::vector<bool>>(const std::string &, std::vector<bool> &,
                                      const ParamGetOptions &) const;
template void
RuntimeParams::get<std::vector<int>>(const std::string &, std::vector<int> &,
                                     const ParamGetOptions &) const;
template void RuntimeParams::get<std::vector<double>>(
    const std::string &, std::vector<double> &, const ParamGetOptions &) const;
template void
RuntimeParams::get<std::vector<std::string>>(const std::string &,
