...
/// @brief Logging class with static members for controlling message severity
/// and verbosity.
class logger {
public:
  ...
  enum class LogLevel {
    FATAL =
        0, ///< Unrecoverable error. Logs to err_stream then throws FatalError.
  ...
  /// @param log_stream Destination for INFO/NOTE/DEBUG messages (default:
  /// std::cout).
  static void set_log_stream(std::ostream &log_stream);

  /// @brief Redirect error output stream.
  /// @param err_stream Destination for WARNING/FATAL messages (default:
  /// std::cerr).
  static void set_err_stream(std::ostream &err_stream);

  /// @brief Set the minimum verbosity level. Messages above this level are
  /// suppressed.
  /// @param level The new verbosity threshold.
  static void set_verbosity(LogLevel level);

  /// @brief Set the minimum verbosity level using an integer. Valid range is
  /// 0-9.
  /// @param level The new verbosity threshold as an integer (0=FATAL,
  /// 1=WARNING, 2=NOTE, 3=INFO, 9=DEBUG).
  ...
  /// @param args The components of the message to log. These will be streamed
  /// together into a single message.
  /// @note It is preferred to use the level-specific logging functions (e.g.
  /// `fatal`, `warning`, etc.).
  ...
  /// @param args The components of the message to log. These will be streamed
  /// together into a single message.
  template <typename... Args> static void note(Args &&...args) {
    log(LogLevel::NOTE, std::forward<Args>(args)...);
  }
...
private:
  /// @brief Core logging function that routes messages based on log level and
  /// current verbosity.
