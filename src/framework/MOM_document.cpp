...
DocFileWriter::DocFileWriter(const std::string &doc_file_base, bool complete, bool minimal, bool layout, bool debugging)
    : doc_file_base_(doc_file_base), complete_(complete), minimal_(minimal), layout_(layout), debugging_(debugging) {}
...
DocFileWriter::~DocFileWriter() {
  ...
  if (file_all_.is_open())
    file_all_.close();
  if (file_short_.is_open())
    file_short_.close();
  if (file_layout_.is_open())
    file_layout_.close();
  if (file_debugging_.is_open())
    file_debugging_.close();
}
...
void DocFileWriter::open_files() {
  ...
  if (layout_ && !file_layout_.is_open()) {
    ...
    file_layout_ << "! This file was written by the model and records the layout " << "parameters used at run-time.\n";
  }
...
std::string DocFileWriter::define_string(const std::string &varname, const std::string &valstring,
...
std::string DocFileWriter::undef_string(const std::string &varname, const std::string &units) const {
...
void DocFileWriter::write_message_and_desc(const std::string &mesg, const std::string &desc, bool value_was_default,
  ...
  while (start < processed_desc.size()) {
    ...
    if (write_all)
      file_all_ << comment_line << "\n";
    if (write_short)
      file_short_ << comment_line << "\n";
    if (write_lay)
      file_layout_ << comment_line << "\n";
    if (write_dbg)
      file_debugging_ << comment_line << "\n";
  }
}
...
bool DocFileWriter::mesg_has_been_documented(const std::string &varname, const std::string &mesg) {
  std::string full_name = block_prefix_ + varname;

  auto it = documented_.find(full_name);
  if (it != documented_.end()) {
    if (it->second != mesg) {
      std::cerr << "WARNING: Inconsistent documentation for parameter " << full_name << "\n"
                << "  Previous: " << it->second << "\n"
                << "  New:      " << mesg << "\n";
    }
    return true;
...
void DocFileWriter::doc_param(const std::string &varname, const std::string &desc, const std::string &units, bool val,
                              std::optional<bool> default_val, const DocParamOptions &opts) {
  open_files();
  if (!files_are_open_)
    return;
  ...
  if (mesg_has_been_documented(varname, mesg))
    return;
  write_message_and_desc(mesg, desc, equals_default, opts.layout_param, opts.debugging_param);
}
...
void DocFileWriter::doc_param(const std::string &varname, const std::string &desc, const std::string &units, double val,
                              std::optional<double> default_val, const DocParamOptions &opts) {
  open_files();
  if (!files_are_open_)
    return;
  handle_module_transition(opts.module);
...
void DocFileWriter::doc_param(const std::string &varname, const std::string &desc, const std::string &units,
                              const std::string &val, std::optional<std::string> default_val,
...
void DocFileWriter::doc_param(const std::string &varname, const std::string &desc, const std::string &units,
                              const std::vector<bool> &vals, std::optional<std::vector<bool>> default_val,
                              const DocParamOptions &opts) {
  open_files();
  if (!files_are_open_)
    return;
...
void DocFileWriter::doc_param(const std::string &varname, const std::string &desc, const std::string &units,
                              const std::vector<double> &vals, std::optional<std::vector<double>> default_val,
                              const DocParamOptions &opts) {
  open_files();
  if (!files_are_open_)
    return;
...
void DocFileWriter::doc_param(const std::string &varname, const std::string &desc, const std::string &units,
                              const std::vector<std::string> &vals, std::optional<std::vector<std::string>> default_val,
                              const DocParamOptions &opts) {
  open_files();
  if (!files_are_open_)
    return;
...
void DocFileWriter::doc_module(const std::string &modname, const std::string &desc, bool layout_mod, bool debugging_mod,
                               bool all_default) {
  open_files();
  if (!files_are_open_)
    return;
...
void DocFileWriter::doc_openBlock(const std::string &blockName, const std::string &desc) {
  open_files();
  if (!files_are_open_)
    return;
